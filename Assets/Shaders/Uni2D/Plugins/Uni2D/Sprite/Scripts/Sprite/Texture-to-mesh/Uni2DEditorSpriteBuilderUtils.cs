...
public static class Uni2DEditorSpriteBuilderUtils 
{
	...
	{	
		...
		else
		{
			...
			if (rMesh2D != null && rMesh2D.shapes!=null)
			{
				foreach (Uni2DMeshShape2D rShape in rMesh2D.shapes)
				{
	...
	{			
		// Instantiate the prefab
		Debug.Log("Saving prefab:" + a_rPrefab.name);
	...
	{
		a_rPrefabPath = System.IO.Path.GetDirectoryName(AssetDatabase.GetAssetPath(a_rPrefab));
